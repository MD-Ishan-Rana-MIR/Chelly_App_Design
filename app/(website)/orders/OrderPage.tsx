...
const OrdersPage = () => {
  ...
  const { data, isLoading, isFetching, error } = useAllOrderQuery({
    page,
    per_page: perPage,
  });
  ...
  return (
    <MaxWidth>
      ...
      <div
        className={`mx-auto py-10 transition-opacity print:hidden ${isFetching ? "opacity-60" : "opacity-100"}`}
      >
        {/* HEADER */}
        <div className="mb-8">
            ...
            <div className="grid gap-4 md:hidden">
              {orders.map((order: Order) => (
                ...
                >
                  ...
                  <div>
                    <span className=" py-1 rounded-full text-xs font-semibold">
                      Payment Status
                    </span>
                    <span
                      className={`px-3 py-1 rounded-full text-xs font-semibold ${getStatusStyle(order.status)}`}

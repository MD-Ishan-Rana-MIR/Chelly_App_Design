...
import { useOnlyGetBreakFastQuery } from '@/app/redux/collectionApi';
import Image from 'next/image';
import Link from 'next/link';
import { useRouter } from 'next/navigation';


export default function BreakfastProduct() {


    const router = useRouter();

    const { data } = useOnlyGetBreakFastQuery({})
        ...
        <section className="mx-auto font-sans md:mb-18 mb-8 ">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
                {breakfastData.map((item) => (
                    <div
                        key={item.id}
                        onClick={() => { router.push(`/food/${String(item?.id)}`) }}
            ...
            <div className="md:mt-16 mt-8 flex justify-center">
                <Link href={`/collections/${1}`} className="bg-[#0f621a] cursor-pointer font-semibold  hover:bg-[#0a4612] text-white  py-3 px-8 transition-colors duration-200 text-sm tracking-wide">
                    View All
                </Link>
            </div>
        </section>
